const testText = `
   ...
   Сильвио встал и вынул из картона красную шапку с золотою кистью, с галуном (то, что французы называют bonnet de police[3]); он ее надел; она была прострелена на вершок ото лба.
...
const setWeightDistribution = (textPair: string[][]) => {
    const weightDistributionDic: {
        [key: string]: string[]
    } = {};

    textPair.forEach( (pair) => {
        weightDistributionDic[pair[0]] = [];
    });

    textPair.forEach( (pair, pairIndex) => {
        weightDistributionDic[pair[0]].push(pair[1]);
    });

    return weightDistributionDic

}


const splitTextToList = () => testText.split(" ");

const makePair = () => {
    const pair: string[][] = [];
    const splittedText = splitTextToList();
    splittedText.reduce((acc: string[][], value: string, index: number, array: string[]) : string[][] => {
        acc.push(array.slice(index, index + 2))
        return acc;
    }, pair)
    pair.length = pair.length - 1;
    return pair;
}

const getFirstWord = () => {
    const parsedText = testText.trim().split(" ");
    const wordsWithCapitalChar = parsedText.filter(textKey => {
        const reg = /[-!$%^&*()_+|~=`{}\[\]:";'<>?,.\/]/;
...
export const initModel = () => {
   ...
   const dic = setWeightDistribution(pair);
   let entityOfWords = 500;


   const firstWord = getFirstWord();
   const chain: string[] = [firstWord];

    let prevWord = chain[0];
    while(entityOfWords >= 0){
        const nextWord = dic[prevWord][Math.floor(Math.random() * dic[prevWord].length)]
        chain.push(nextWord);
        prevWord = nextWord
        --entityOfWords;
    }
